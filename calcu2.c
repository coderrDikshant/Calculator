...
#define IDC_BUTTON_E 224
#define IDC_BUTTON_ASIN 225
#define IDC_BUTTON_ACOS 226
#define IDC_BUTTON_ATAN 227
#define IDC_BUTTON_SQUARE 228
#define IDC_BUTTON_CUBE 229
#define IDC_BUTTON_RECIPROCAL 230
#define IDC_BUTTON_SQRT 231
#define IDC_BUTTON_LOG 232
#define IDC_BUTTON_FACTORIAL 233
...
int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow)
{
    ...
    window_var.hbrBackground = (HBRUSH)(COLOR_WINDOW+8);
    ...
    switch (message) {
        case WM_CREATE:
        {
            ...
            CreateWindow(
                "BUTTON", "=",
                WS_VISIBLE | WS_CHILD,
                BUTTON_ROW_X + 60* 3 + ROW_GAP*3, BUTTON_ROW_Y * 6, BUTTON_WIDTH, BUTTON_HEIGHT * 2 + 3,
            ...
            CreateWindow(
                ...
                hwnd, (HMENU)IDC_BUTTON_E, GetModuleHandle(NULL), NULL);
            CreateWindow(
                "BUTTON", "Sqrt",
                WS_VISIBLE | WS_CHILD,
            ...
            if (HIWORD(wParam) == BN_CLICKED) {
                switch (LOWORD(wParam)) {
                    ...
                    case IDC_BUTTON_FACTORIAL:
                        if (!FinalCalcFlag) {
                        LeftOperand = factorial((int)LeftOperand);
...
void CalculateOperand() {
    if (OperandType == LEFT) {
        // Check if the string represents the mathematical constant "e"
        if (strcmp(strValue, "e") == 0) {
            LeftOperand = 2.7182818284590452; // Set LeftOperand to the value of "e"
        } else {
...
void CalculateLogarithm() {
    double result = log(OperandType == LEFT ? LeftOperand : RightOperand);
    gcvt(result, MAX_DIGITS, strValue);
    RemoveStringDecimal();
}
...
void CalculateSquareRoot() {
    ...
    if (OperandType == LEFT) {
        LeftOperand = result; // Update the LeftOperand with the square root
    } else {
        RightOperand = result; // Update the RightOperand with the square root
    }
    gcvt(result, MAX_DIGITS, strValue);
    RemoveStringDecimal();
}
...
void CalculateSquare() {
    ...
    if (OperandType == LEFT) {
        LeftOperand = result; // Update the LeftOperand with the square
    } else {
        RightOperand = result; // Update the RightOperand with the square
    }
    gcvt(result, MAX_DIGITS, strValue);
    RemoveStringDecimal();
}
...
void CalculateCube() {
    ...
    if (OperandType == LEFT) {
        LeftOperand = result; // Update the LeftOperand with the cube
    } else {
        RightOperand = result; // Update the RightOperand with the cube
    }
    gcvt(result, MAX_DIGITS, strValue);
    RemoveStringDecimal();
}
